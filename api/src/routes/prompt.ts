...
const prabowoPrompt = async (messages: { role: string, content: string }[]) => {
  const query = messages[messages.length - 1].content
  const emresp = await fetch('https://api.openai.com/v1/embeddings', {
    body: JSON.stringify({
      model: 'text-embedding-ada-002',
  ...
  let records: any[] = JSON.parse(readFileSync(`${__dirname}/../../docs/objects/prabowo.obj.json`).toString())
  records = records.map(record => ({
    ...record,
    'similarity': cosineSimilarity(record.embeddings, embeddings)
  }))
...
export default async function handler(req: Request, res: Response) {
  ...
  if (candidate === 'prabu') {
    return res.json({
      messages: await prabowoPrompt(conv),
      model: 'gpt-3.5-turbo',
      temperature: 0.05
    })
  }
