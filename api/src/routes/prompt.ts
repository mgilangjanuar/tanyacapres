...
const aniesPrompt = async (messages: { role: string, content: string }[]) => {
  ...
  return [
    {
      role: 'system',
      content: crypt.encrypt(`You are a helpful assistant that helps people find information from the parsed PDFs. You are only can answer the question based on the documents and you can ignore the question if there is no data from the PDFs. Here are the top 3 results:
